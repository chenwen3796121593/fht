...
跨市场实时行情看板 — A股指数 + 大宗商品 + 贵金属 + 宏观指标 + 自选 + 群聊 + 预警 + VIP
...
| 📈 **自选** | TradingView K线图(缩放/拖拽/十字光标) + 成交量 + 自选列表 + 分时/日线/全部 |
| 📊 **指标** | M1/M2/贷款/准备金 SVG四宫格 + 上证月线叠加 + 24h localStorage缓存 |
| 📰 **新闻** | RSS股票/商品 + 新浪宏观 + 星级排序 + 5min自动刷新 |
| 💬 **聊天** | Supabase实时群聊 + 语音消息 + 48个Lucide表情 + 消息缓存秒开 + 日期显示 |
| 🔔 **预警** | 涨跌幅/价格触发 + 推送/震动/语音通知 + 5分钟防骚扰 |
| 💰 **贵金属** | 沪金/沪银/铂金/钯金 实时销售价 + AI商品预测(Supabase实时) + 微信二维码 |
| 👑 **VIP** | 申请审核 + 会员登录 + 管理员JWT校验 + 策略发布 + 商品股票趋势预测 |
...
├── components/      TopBar(电子时钟) / MarketBar / Watchlist / StockChart / ErrorBoundary / Skeleton
...
| 商品AI预测 | Supabase commodity_rankings(实时订阅) | 自动 |
