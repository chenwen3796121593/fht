...
export async function getGoldKlines(period = 'd') {
  if (period === '1h' || period === '4h') return await kraken(period)
  if (period === 'm') { const d = await sinaDaily(); return d ? toMonthly(d) : null }
  if (period === 'w') { const d = await sinaDaily(); return d ? toWeekly(d) : null }
  return await sinaDaily()
}
