...
export default function VipPage({ onNavigate }) {
  ...
  const loadApps = () => {
    setLoadingApps(true)
    getSB().then(c => c.from('vip_applications').select('*').order('created_at', { ascending: false })
  ...
  const handleApprove = async (app) => {
    ...
    try {
      const client = await getSB()
      await client.from('vip_users').insert({ username: app.phone, password: pwd, phone: app.phone })
      await client.from('vip_applications').update({ status: 'approved' }).eq('id', app.id)
      handleRefresh()
  ...
  const handleReject = async (app) => {
    try {
      const client = await getSB()
      await client.from('vip_applications').update({ status: 'rejected' }).eq('id', app.id)
      handleRefresh()
    } catch(e) {}
  }

  const handleSaveStrategy = async () => {
    if (!strategyDraft.trim()) return
    setSavingStrat(true)
    try {
      const client = await getSB()
      const { data } = await client.from('vip_strategies').select('id').order('updated_at', { ascending: false }).limit(1)
      if (data && data.length > 0) {
        await client.from('vip_strategies').update({ content: strategyDraft.trim(), updated_at: new Date().toISOString() }).eq('id', data[0].id)
      } else {
        await client.from('vip_strategies').insert({ content: strategyDraft.trim() })
      }
    ...
    <div className="bg-[#0A0F14] h-full overflow-y-auto">
      ...
      <div className="px-4 pt-4 pb-3">
        <div className="bg-[#12161C] border border-[#F59E0B]/30 rounded-xl p-4 flex flex-col gap-3">
          ...
          <button onClick={handleSaveStrategy} disabled={savingStrat||!strategyDraft.trim()} className="py-2 rounded-md bg-[#F59E0B] text-[#0A0F14] text-sm font-bold disabled:opacity-40">{savingStrat?'发布中...':'发布策略'}</button>
        </div>
      </div>
      ...
      <div className="px-4 flex flex-col gap-2 pb-8">
        ...
        {applications.map(app=>(
          <div key={app.id} className="bg-[#12161C] rounded-lg p-3.5 flex items-center justify-between">
            <div><div className="text-sm text-[#F0F2F5]">{app.phone}</div>{app.reason&&<div className="text-[11px] text-[#6B7280] mt-0.5">{app.reason}</div>}<div className="text-[10px] text-[#4D545C] mt-1">{new Date(app.created_at).toLocaleDateString('zh-CN')}<span className={`ml-2 ${app.status==='pending'?'text-[#F59E0B]':app.status==='approved'?'text-[#22C55E]':'text-[#EF4444]'}`}>{app.status==='pending'?'待审核':app.status==='approved'?'已通过':'已拒绝'}</span></div></div>
            {app.status==='pending'&&<div className="flex gap-2"><button onClick={()=>handleApprove(app)} className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#22C55E]/20 text-[#22C55E] hover:bg-[#22C55E]/30"><Check size={16}/></button><button onClick={()=>handleReject(app)} className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#EF4444]/20 text-[#EF4444] hover:bg-[#EF4444]/30"><X size={16}/></button></div>}
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="bg-[#0A0F14] h-full overflow-y-auto">
      ...
      <div className="px-4 pt-5 flex flex-col items-center gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-[#F59E0B]/20 flex items-center justify-center flex-shrink-0"><Crown size={24} className="text-[#F59E0B]"/></div>
          <div>
            <div className="text-base font-bold text-[#F0F2F5] leading-tight">VIP 专区</div>
            <div className="text-xs text-[#8D949E]">{currentUser}</div>
          </div>
        </div>

        {strategy && (
          <div className="w-full bg-[#12161C] border border-[#F59E0B]/30 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3"><Edit3 size={14} className="text-[#F59E0B]"/><span className="text-xs font-semibold text-[#F59E0B]">今日策略</span></div>
  ...
  return (
    <div className="bg-[#0A0F14] h-full overflow-y-auto">
      ...
      <div className="px-4 pb-4"><div className="rounded-xl px-1 py-3.5">
      ...
      <div className="px-4 pb-4"><div className="bg-[#12161C] border border-[#242B33] rounded-xl p-4 flex flex-col gap-3">
        ...
        {submitted?<div className="text-center py-4 text-sm text-[#22C55E] font-medium">申请已提交，等待审核 ✓</div>:<>
      ...
      <div className="px-4 pb-8"><div className="bg-[#12161C] border border-[#242B33] rounded-xl p-4 flex flex-col gap-3">
        <div className="flex items-center gap-2"><LogIn size={14} className="text-[#F59E0B]"/><span className="text-xs font-semibold text-[#F0F2F5]">VIP 登录</span></div>
        ...
        <button onClick={handleLogin} disabled={!username.trim()||!password.trim()} className="w-full py-2.5 rounded-md bg-[#F59E0B] text-[#0A0F14] text-sm font-bold disabled:opacity-40">登录</button>
      </div></div>
    </div>
  )
}
